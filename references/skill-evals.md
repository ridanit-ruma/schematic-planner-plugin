...
| 3 | Implementation Plans only in `plans` | `create_plan` uses `folder: "plans"`, inventory confirms it, and `Source-Specs` names the approved Spec. | The implementation Plan appears elsewhere, lacks provenance, or executable work is written onto the Spec. |
...
| 5 | Fresh evidence before task completion | The task remains `in_progress` until fresh focused and broader checks pass and `evidence-<task-slug>` identifies the verified commit/result. | The task becomes `done` with missing, stale, or mismatched command evidence. |
